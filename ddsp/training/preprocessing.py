...
class F0MIDILoudnessPreprocessor(F0LoudnessPreprocessor):
  """Resamples and scales 'f0_hz', 'f0_hz_midi 'and 'loudness_db' features."""

  def call(self, loudness_db, f0_hz, f0_hz_midi) -> [
      'f0_hz', 'loudness_db', 'f0_hz_midi', 'f0_scaled', 'ld_scaled', 'f0_midi_scaled']:
    """Compute power on the fly if it's not in the inputs."""
  ...
  @staticmethod
  def invert_scaling(f0_scaled, ld_scaled, f0_midi_scaled):
    """Puts scaled f0, loudness, and power back to hz & db scales."""
    f0_hz = ddsp.core.midi_to_hz(F0_RANGE * f0_scaled)
    loudness_db = (ld_scaled - 1.0) * LD_RANGE
